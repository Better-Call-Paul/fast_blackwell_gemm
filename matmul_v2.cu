...
__global__ void basic_tcgen_matmul(const __nv_bfloat16 *A, const __nv_bfloat16 *B, __nv_bfloat16 *C, int M, int N, int K, const __grid_constant__ CUtensorMap a_tensor_map, const __grid_constant__ CUtensorMap b_tensor_map)
{
...
void matmul_v2(const __nv_bfloat16 *A, const __nv_bfloat16 *B, __nv_bfloat16 *C, int M, int N, int K)
{
    ...
    // make maps on host
    init_tmap_2d_simple(&a_tensor_map, A, M, K, BM, BK, CUtensorMapSwizzle::CU_TENSOR_MAP_SWIZZLE_128B);
    init_tmap_2d_simple(&b_tensor_map, B, K, N, BK, BN, CUtensorMapSwizzle::CU_TENSOR_MAP_SWIZZLE_128B);

    int grid = (M * N) / (BM * BN);
    int TB_size = NUM_WARPS * WARP_SIZE;
